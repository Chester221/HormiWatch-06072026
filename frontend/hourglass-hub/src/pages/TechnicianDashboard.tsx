...
const TechnicianDashboard = () => {
  ...
  // ✅ Solo Admin no puede ver este dashboard (va a /control-usuarios)
  if (isAdmin) {
    return (
      <DashboardLayout>
  ...
  const tasks = useMemo(() => {
    if (isManager) {
      return allTasks; // Manager ve todas
    }
  ...
  const projects = useMemo(() => {
    if (isManager) {
      return allProjects; // Manager ve todos
    }
    // Técnico ve solo proyectos donde está asignado
      ...
      <div className="mb-8">
        ...
        <p className="mt-1 text-muted-foreground">
          {(() => {
            ...
            if (pendingCount === 0) return "Todas las tareas están completadas. Buen trabajo.";
      ...
      {isLoading ? (
        ...
        <>
          <div className="mb-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            <MetricCard
              title={isManager ? "Tareas Registradas" : "Mis Tareas"}
            ...
            />
            <MetricCard
              title={isManager ? "Horas Registradas" : "Mis Horas"}
            ...
            />
            <MetricCard
              title={isManager ? "Proyectos" : "Mis Proyectos"}
              value={metrics.proyectos.count}
              subtitle={isManager ? "Total registrados" : "Proyectos asignados"}
              icon={FolderKanban}
