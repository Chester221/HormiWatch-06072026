...
<div className="grid grid-cols-2 md:grid-cols-5 gap-3">
  {[
    ...
    >
      {/* Círculo decorativo azul tenue (igual que MetricCardMG) */}
      ...
      {/* Ícono en la esquina (igual que MetricCardMG) */}
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="rounded-xl border border-border bg-card p-3">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <Input placeholder="Buscar por nombre o email..." value={searchQuery} onChange={e => { setSearchQuery(e.target.value); setCurrentPage(1); }} className="pl-9 h-9 text-xs bg-background" />
            </div>
            <div className="flex gap-2">
              <Select value={roleFilter} onValueChange={setRoleFilter}>
                <SelectTrigger className="h-9 w-[120px] text-xs"><SelectValue placeholder="Rol" /></SelectTrigger>
                <SelectContent>
              ...
              <Select value={statusFilter} onValueChange={setStatusFilter}>
                <SelectTrigger className="h-9 w-[120px] text-xs"><SelectValue placeholder="Estado" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">Todos</SelectItem>
        ...
        <div className="flex items-center justify-between pt-3 mt-3 border-t border-border/50">
          ...
          <div className="flex items-center gap-0.5">
            ...
            <Button 
              ...
              onClick={() => setDeleteDialog({ open: true, user })} title="Eliminar"
            >
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <Input value={editFullName} onChange={e => setEditFullName(e.target.value)} className="bg-muted/30 border-border h-10 rounded-lg text-sm" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input value={editEmail} onChange={e => setEditEmail(e.target.value)} className="bg-muted/30 border-border h-10 rounded-lg text-sm" />
            </div>
            <div>
              <Label className="text-xs font-medium flex items-center gap-1.5 mb-1.5">
                <Phone className="h-3.5 w-3.5 text-muted-foreground" /> Teléfono
              </Label>
              <Input value={editPhone} onChange={e => setEditPhone(e.target.value)} placeholder="+58 412 413 4891" className="bg-muted/30 border-border h-10 rounded-lg text-sm font-mono" />
            </div>
          </div>
          ...
          <div>
            ...
            <Input value={editCedula} onChange={e => setEditCedula(e.target.value)} placeholder="V-12345678" className="bg-muted/30 border-border h-10 rounded-lg text-sm" />
          </div>
        </div>

        {/* Footer */}
        <DialogFooter className="p-5 pt-0 gap-2">
          <Button variant="outline" onClick={() => setEditModal({ open: false, user: null })} className="rounded-lg h-10 px-4 text-sm border-border flex-1">
            Cancelar
          </Button>
          <Button onClick={handleSaveEdit} disabled={isSaving} className="rounded-lg h-10 px-4 gap-2 text-white text-sm flex-1" style={{ backgroundColor: HORMI_BLUE }}>
